...
/***************************************************************************/
    ...
    // Set data rate
    nRF24_SetDataRate(nRF24_DR_250kbps); //  2Mbps);
    ...
    // Configure RX PIPES
    static const uint8_t nRF24_ADDR[] = { 'E', 'S', 'B', 'X', 'Z' };
    ...
    static const uint8_t nRF24_ADDR2[] = { 'Y' };
    nRF24_SetAddr(nRF24_PIPE2, nRF24_ADDR2); // program address for pipe
...
void nRF24_loop(void)
{
	...
	if (nRF24_GetStatus_RXFIFO() != nRF24_STATUS_RXFIFO_EMPTY)
	{
		...
		// Print a payload contents to UART
		String msg_str = "RCV PIPE#";
		HelpersLib::num2str(&msg_str, pipe);
		msg_str += " LEN:";
		HelpersLib::num2str(&msg_str, payload_length);
		msg_str += " PAYLOAD:>";

		for (uint8_t i=0; i < payload_length; i++)
		{
			HelpersLib::hex2str(&msg_str, &nRF24_payload[i]);
		}
