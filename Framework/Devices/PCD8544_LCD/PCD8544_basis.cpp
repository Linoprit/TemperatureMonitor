...
PCD8544_basis::PCD8544_basis(PCD8544_socket *socket)
{
	this->socket = socket;

	socket->activate_CS();
	socket->do_reset();
	socket->deactivate_CS();
...
#define DLY 10
	Common::delay(DLY);
	socket->write_byte(0x21, PCD8544_COMMAND); // switch to extended commands
	Common::delay(DLY);

	socket->write_byte(0xc0, PCD8544_COMMAND); // set Vop (contrast)
	Common::delay(DLY);

	socket->write_byte(0x06, PCD8544_COMMAND); // set temp-coefficient
	Common::delay(DLY);

	socket->write_byte(0x13, PCD8544_COMMAND); // bias mode 1:48
	Common::delay(DLY);

	socket->write_byte(0x20, PCD8544_COMMAND); // horizontal addressing
	Common::delay(DLY);

	socket->write_byte(0x0c, PCD8544_COMMAND); // enable normal display
	Common::delay(DLY);

	socket->write_byte(0x09, PCD8544_COMMAND); // turn all segments on
	Common::delay(500); // Wait for device to settle

	socket->write_byte(0x08, PCD8544_COMMAND); // display blank
...
void PCD8544_basis::draw_bitmap(uint8_t column,
		uint8_t line,
...
void PCD8544_basis::write_string(uint8_t column,
		uint8_t line,
		char 	 *ptr_to_text,
		uint8_t mode)
{
		...
		uint8_t mode)
{
	...
	for(i=0;i<3;i++)
	{
		...
		for(j=0; j<16; j++){
			...
			socket->write_byte( (mode == MENU_NORMAL)? ch_dat : (ch_dat^0xff), 1);
		}
	}
}
#endif

void PCD8544_basis::write_char(uint8_t character,
		uint8_t mode)
{
	uint8_t line;
	uint8_t pattern;
...
void PCD8544_basis::set_line_n_column(uint8_t column,
		uint8_t line)
{
	socket->write_byte(0x40 | line, 0); // 0 <= line <= 5
	socket->write_byte(0x80 | column, 0); // 0 <= X <= 83
}
...
void PCD8544_basis::clear(void)
{
	...
	socket->write_byte(0x0c, 0); // Normal Mode
	socket->write_byte(0x80, 0); // set x address to 0

	for (i=0; i<(DISPLAY_WIDTH_px * DISPLAY_HEIGHT_px / 8); i++)
		socket->write_byte(0, 1);
}
